...
const Status HeapFileScan::startScan(const int offset_, const int length_, const Datatype type_, const char *filter_, const Operator op_)
{
    if (!filter_)
...
const Status HeapFileScan::scanNext(RID &outRid)
{
    ...
    outRid = NULLRID;
	//if curPage is null
	if (curPageNo == -1) return FILEEOF;//TODO
    if (curPage == NULL)
    {
	//	cout<<"1\n";//TODO
        curPageNo = headerPage->firstPage;
	...
	//cout<<endl;//TODO
	//cout<<"nextPageNo: "<<nextPageNo<<endl;//TODO
	//cout<<"curRec,pageNo: "<<curRec.pageNo <<" slotNo: " 
	//<<curRec.slotNo<<endl;//TODO
    // start from curRec instead of firstRecord
	...
	//when curRec is not null
		...
		if (status != OK)
		{
			...
			while(status != OK)
			{
				...
				if (status != OK) return status;
				curPageNo = nextPageNo;
				status = bufMgr->readPage(filePtr, curPageNo,
						curPage);
				if (status != OK) return status;
				status = curPage->firstRecord(curRec);
			}
	...
	nextRid = curRec;
	//cout<<"curRec: "<<curRec.pageNo<<" "<<curRec.slotNo<<endl;//TODO
	while (true)
	{
	//printf("1\n");//TODO
		do
		{
			//nextRid = curRec;//TODO:added
			curRec = nextRid;
			//tmpRid = nextRid;
			...
			if (matchRec(rec))
			{
				//curRec = tmpRid;
				outRid = curRec;
				// cout << "RID pageNo: " << tmpRid.pageNo << " slotNo: " << tmpRid.slotNo << endl;
				//cout<<"e12\n";
				return status;
			}
			status = curPage->nextRecord(curRec, nextRid);
		} while (status == OK);
		//after loop through the current record, go to the next
		//page
		status = curPage->getNextPage(nextPageNo);
		if (status != OK)
		{
	...
	// This return value is checked by the tester once the file gets to its end
	return FILEEOF;
	// TODO END
	*/
	 Status  status = OK;
...
const Status InsertFileScan::insertRecord(const Record &rec, RID &outRid)
{
	...
	else
	{
		...
		// allocPage will pin newPage so do not readPage again
		curPageNo = newPageNo;
		curPage = newPage;
		curDirtyFlag = false;
