   ...
   cout << "inserted " << num << " records into file dummy.02 successfully" << endl;
    ...
    else 
    {
		...
		for (i=0;i<num;i=i+11)
		{
    	    ...
    	    if (status != OK) error.print(status);
			//printf("ridArray[i].slotNo: %d\n",
			//ridArray[i].slotNo);//TODO
    ...
    else 
    {
        ...
        while ((status = scan1->scanNext(rec2Rid)) != FILEEOF)
        {
		cout<<"rec2Rid: "<<rec2Rid.pageNo<<" "<<rec2Rid.slotNo
		<<endl;//TODO
            status = scan1->getRecord(dbrec2);
            if (status != OK) break;
	...
	else 
	{
		...
		for(i = 0; i < num; i++)
		{
			...
			cout<<"scan1: i:"<<i<<endl;//TODO
			cout<< "rec2Rid.pageNo: "<<rec2Rid.pageNo<<
				" rec2Rid.slotNo: "<<rec2Rid.slotNo<<endl;//TODO

			status = scan2->scanNext(rec2Rid);
			if (status == OK) count++;
			else if (status != FILEEOF) error.print(status);
			cout<<"scan2: i:"<<i<<endl;//TODO
			cout<< "rec2Rid.pageNo: "<<rec2Rid.pageNo<<
				" rec2Rid.slotNo: "<<rec2Rid.slotNo<<endl;//TODO
		...
		while ((status = scan1->scanNext(rec2Rid)) != FILEEOF)
		{
			cout<<"rec2Rid pageNo: "<<rec2Rid.pageNo<< 
				"rec2Rid slotNo: "<<rec2Rid.slotNo<<endl;//TODO
														 // reconstruct record i
			status = scan1->getRecord(dbrec2);
			cout<<"dbrec2 int: "<<*(int*)dbrec2.data<<endl;//TODO
			if (status != OK) break;
