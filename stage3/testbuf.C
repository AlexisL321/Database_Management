  ...
  struct stat statusBuf;
    ...
    for (i = 0; i < num/3; i++) 
    {
	  ...
	  //printf("1\n");//TODO
	   ...
	   cout <<"cmp1:"<< (char*)cmp << endl;
     ASSERT(memcmp(page, &cmp, strlen((char*)&cmp)) ==0);//TODO: shouldn't comment it out
      CALL(bufMgr->readPage(file2, pageno2, page2));
      sprintf((char*)&cmp, "test.2 Page %d %7.1f", pageno2, (float)pageno2);
	  cout<<"page2:" <<(char*)page2<<endl;
	  cout<<"cmp2:"<<(char*)cmp<<endl;
	  ...
	  cout<<"page3:"<<(char*)page3<<endl;
	  cout<<"cmp3:"<<(char*)cmp<<endl;
