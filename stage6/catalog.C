				  ...
				  AttrDesc &record)
{
  ...
  //This while loop finds the next record that that has the same
  //attrName as passed in parameter and put the record in the
  //record reference passed in the parameter.
  while((status = hfs->scanNext(rid)) == OK) 
  {
    if ((status = hfs->getRecord(rec)) != OK) return status;
    assert(sizeof(AttrDesc) == rec.length);
	//TODO: does this memcpy actually copied attrLen?
    memcpy(&record, rec.data, rec.length);
