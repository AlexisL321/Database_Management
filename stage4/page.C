...
cout<< "this is printing out dumpPage()"<<endl;
cout<<endl;
...
*/
	//TODO: print out each record in the page
	cout << "each record in the page:" <<endl;
	RID firstRec;
	Status status = firstRecord(firstRec);
	if (status != OK) {
		cout<<"no record in this page" <<endl;
		return;
	}
	RID nextRec;
	while (nextRecord(firstRec, nextRec) == OK) {
		Record curRec;
		getRecord(firstRec, curRec);
		cout<<"data: "<< *(int*)curRec.data<< "  " << "length:" <<
			curRec.length<<endl;
		cout<<"RID.pageNo " << firstRec.pageNo << "RID.slotNo " <<
			firstRec.slotNo<<endl;
		firstRec = nextRec;
	}
...
const Status Page::insertRecord(const Record & rec, RID& rid)
{
	...
	else
	{
		...
		rid = tmpRid;
		//printf("insertRecord: pageNo: %d, slotCnt: %d, slotNo: %d\n",
		//curPage, slotCnt, rid.slotNo);//TODO
